...
{
  return static_cast<Protocol *>(_p)->state();
}
...
{
  return static_cast<Protocol *>(_p)->host().c_str();
}
  ...
  if (elisteners != this->listeners.end())
  {
    for (auto elistener : elisteners->second)
    {
      auto bridge = std::find_if(
          ...
          [=](std::pair<kuzzle_event_listener, SharedEventListener> vt) { return vt.second == elistener; });
