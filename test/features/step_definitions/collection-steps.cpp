...
THEN("^the mapping of \'([^\"]*)\' should be updated$")
{
  ...
  std::string mapping = ctx->kuzzle->collection->mapping(
...
THEN("^the specifications of \'([^\"]*)\' should be updated$")
{
  ...
  std::string specifications = ctx->kuzzle->collection->specifications(
      ctx->index, collection_id);
...
THEN("^the specifications of \'([^\"]*)\' must not exist$")
{
  ...
  std::string res = ctx->kuzzle->collection->specifications(
      ctx->index, collection_id);
